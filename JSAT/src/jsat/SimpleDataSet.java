...
public class SimpleDataSet extends DataSet<SimpleDataSet>
{
    ...
    {
      String toReturn = "";
      //dataPoints.forEach((dp) -> { System.out.println(dp); });
      // figure out how to update the version of java so this works better cuz like wtf...??
      
      int numberOfSamples = dataPoints.get(0).numNumericalValues(); //all should have the same number of points...it is ICA, after all. 


      for(int i = 0; i < numberOfSamples; i++) {
        for(DataPoint dp : dataPoints) {
          toReturn += (dp.getValue(i) + ", ");
        }
        toReturn += "\n";
      }
      return toReturn;
